<template>
  <div class="register">
    ...
    <div class="registerform">
      <h2 style="cursor: pointer;">Enroll在线活动报组织平台注册</h2>
      <el-form
        ref="organizerRegisterForm"
        :model="organizerRegisterForm"
        :rules="organizerRegisterRules"
        label-width="95px"
      >
        <el-form-item label="用户名" prop="organizername" class="whiteItem">
          <el-input v-model="organizerRegisterForm.organizername" ref="organizername"></el-input>
        </el-form-item>
        <el-form-item label="密码" prop="password" class="whiteItem">
          <el-input
            :key="passwordType"
            v-model="organizerRegisterForm.password"
            :type="passwordType"
            ref="password"
          ></el-input>
        ...
        <el-form-item label="确认密码" prop="passwordtwo" class="whiteItem">
          <el-input
            :key="passwordTypeTwo"
            v-model="organizerRegisterForm.passwordtwo"
...
export default {
  ...
  data () {
    ...
    const validateOrganizerName = (rule, value, callback) => {
      if (!value) {
    ...
    return {
      ...
      // 登陆表单的校验规则
      organizerRegisterRules: {
        organizername: [
          { required: true, trigger: 'blur', validator: validateOrganizerName }
        ],
        password: [
  ...
  methods: {
    ...
    // 提交注册表单方法
    submitOrganizerFrom () {
      this.$refs.organizerRegisterForm.validate(valid => {
        if (valid) {
          this.$store
            .dispatch('user/organizerRegister', this.organizerRegisterForm)
            .then(() => {
              Message({
                showClose: true,
                message: '注册成功！可以登录了哦！',
              ...
              this.$router.push('organizerLogin')
            })
            .catch(error => {
              console.log(error)
            })
  ...
  mounted: function () {
    ...
    window.onresize()
    // 数据渲染后自动聚焦到输入框
    if (this.organizerRegisterForm.organizername === '') {
      this.$refs.organizername.focus()
    }
  }
}
</script>
